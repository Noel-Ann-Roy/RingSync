...
# ):
#     """
#     num_threads: if set, pins PyTorch's intra-op thread count for this
#     process (see worker_node.py's identical use of torch.set_num_threads
#     for the full explanation). Left as None by default -- meaning
#     PyTorch uses its normal behavior of spreading each operation across
#     all available cores. Passing num_threads=1 here produces a baseline
#     that is directly comparable to a single distributed worker (which is
#     also pinned to 1 thread), isolating the effect of data-parallelism
#     itself from the effect of intra-op multi-threading.
#     """
...
def train(
    ...
    output_dir: where to save the loss history JSON. Defaults to the
    project-root results/ folder for backward compatibility; pass an
    experiment-specific directory to keep each run's output isolated.

    progress_config: the "config" tag used in the RS_PROGRESS sentinel
    lines (see below) -- lets the caller distinguish "baseline" from
    "baseline_singlethread" in live progress tracking.
    """
    if num_threads is not None:
        torch.set_num_threads(num_threads)
